...
- **USER-NAMED ROLE BINDING (`routes/models.ts` directiveBlock)**: when a user types "X 作为自变量 / X as IV" the AI used to silently swap X for its perceived dimensions (e.g. "AI 主播" → perceived anthropomorphism + perceived responsiveness) without ever showing X as a node or disclosing the substitution — user complaint: "我核心的自变量没加进去". New 4-step procedure inside the PRIMARY USER DIRECTIVE block: (A) bilingual-map the named construct; (B) scan EXTRACTED VARIABLES POOL for ≥70%-meaning match against name + alternate names + canonical id — if hit, USE LITERALLY in the named role; (C) only when no match exists (named construct is likely a stimulus context, not a measurable variable) may operationalization via dimensions occur, AND [USER PROMPT FIT] MUST disclose the substitution with the exact pattern "用户指定『<名>』作为<角色>；变量库中无统一的『<名>』构念，故以其感知维度『<dim1>』『<dim2>』... 作为<角色>的操作化" or the model is rejected; (D) literal match wins over dimension expansion when both are possible.
