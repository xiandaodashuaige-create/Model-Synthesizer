...
- **Slice 2 status (in progress)**: Phase 2 added `EdgeMatchedRelationship` + structured `EdgeNoveltyTag` (`{tag, subscore, reason, matchedRelationship}`) so the UI can explain *why* each tag was picked. New read endpoint `GET /sessions/{id}/landscape` (auth-gated by `loadAuthorizedSession`) returns `{landscapeVersion, lastRebuildAt, coverage, relationships[], theoryClusters[], evidencedBackbones[]}` — pure read, no rebuild. New "文献全景" tab (`/sessions/:id/landscape`) renders coverage banner + CR table + theory clusters + backbones; bracket cutoffs (`≥7 saturated / ≥3 established / 1–2 underexplored`) are mirrored from `bracketByOccurrences()` in `lib/innovation-scoring.ts` per the threshold contract in `docs/innovation-taxonomy.md`. Slice 3 (per-model panel + `contributionStatement`) is next.
