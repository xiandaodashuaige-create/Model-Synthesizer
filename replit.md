...
- **Slice 3 status (in progress — UI shipped, contributionStatement still backend-null)**: Shared `<InnovationMetaPanel>` (`components/innovation-meta-panel.tsx`) renders in two variants — `compact` on every candidate model card (`pages/sessions/models.tsx`) and `full` on the model detail page (`pages/sessions/model-detail.tsx`). Hard contract enforced by the panel: (1) the 4 sub-scores (`differentiation` / `gapFit` / `theoreticalSoundness` / `evidenceSupport`) are ALWAYS shown next to the headline `contributionScore` and never collapse; (2) per-edge `EdgeNoveltyTag` rows are individually collapsible with `aria-expanded`/`aria-controls` and reveal `reason` + `matchedRelationship` on expand; (3) `contributionStatement === null` renders an explicit "尚未生成贡献陈述" empty state (NOT 0 / NaN / undefined), and an `innovationMeta === null` whole model renders a single recompute CTA card; (4) `mode === "analysis_only"` renders a prominent amber banner with the coverage % from `computedAgainst.coverageRate`; `stale === true` shows an amber stale badge. Recompute uses `useRecomputeModelInnovation` and invalidates `getGetModelQueryKey(modelId)` + `getListSessionModelsQueryKey(sessionId)` on success. Phase 3 (AI-emitted `contributionStatement` + Reviewer simulator) is next.
