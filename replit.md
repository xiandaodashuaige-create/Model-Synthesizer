...
- **Bug**: "AI chatbot service quality" (Qian Chen 2023) and "AI-chatbot service quality" (Muhammad Farrukh Shahzad 2024) rendered as TWO independent pinnable cards on `/variables`. User pinned both → triggered the parallel-stimulus rule (≥2 pinned stimulus IVs) and inflated the focus-pick set with a duplicate construct. Root cause: the cluster grouping in both client `clusterKey()` and server `canonicalize()` only did `.toLowerCase().replace(/\s+/g, " ").trim()` — hyphens were preserved, so the two names produced different cluster keys / canonicalConstructIds.
- **Fix**: shared 6-step normalization spec applied symmetrically in `normalizeName()` (new export from `lib/focus-selection.ts`) and `canonicalize()` (`routes/variables.ts`): NFKC → lowercase → hyphens/em-dashes/en-dashes/underscores/slashes → space → strip leading `perceived/the/a/an` → drop residual punctuation → collapse whitespace. Same input string MUST produce identical output on both sides; "MUST stay in lock-step" comments at both sites.
- **localStorage v1→v2 migration**: focus-pick keys are persisted as `${type}|${normalized-name}`; tightening normalization would orphan all old picks. `loadFocusedClusterKeys()` now reads `focusVarSelection:v2:${id}`, falls back to v1, re-runs each name half through the new `normalizeName()`, dedupes, persists under v2, and removes the v1 entry. One-shot per session, idempotent if a key was already in canonical form.
- **`/variable-graph` alignment**: the graph route had its own `v.name.toLowerCase().trim()` node-id grouping and `norm()` for hypothesis from/to/via — diverged from the variable-list cluster grouping after the fix. Both now delegate to `canonicalize()` with a lowercase-only fallback for empty results, so graph nodes and list clusters are guaranteed to use identical ids.
