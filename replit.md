...
### Innovation Layer (see `docs/innovation-taxonomy.md` for the full spec)
...
- **Slice 2 status (shipped)**: Phase 2 added `EdgeMatchedRelationship` + structured `EdgeNoveltyTag` (`{tag, subscore, reason, matchedRelationship}`) so the UI can explain *why* each tag was picked. New read endpoint `GET /sessions/{id}/landscape` (auth-gated by `loadAuthorizedSession`) returns `{landscapeVersion, lastRebuildAt, coverage, relationships[], theoryClusters[], evidencedBackbones[]}` — pure read, no rebuild. New "文献全景" tab (`/sessions/:id/landscape`) renders coverage banner + CR table + theory clusters + backbones; bracket cutoffs (`≥7 saturated / ≥3 established / 1–2 underexplored`) are mirrored from `bracketByOccurrences()` in `lib/innovation-scoring.ts` per the threshold contract in `docs/innovation-taxonomy.md`.
- **Slice 3 status (shipped)**: Shared `<InnovationMetaPanel>` (`components/innovation-meta-panel.tsx`) renders in two variants — `compact` on every candidate model card (`pages/sessions/models.tsx`) and `full` on the model detail page (`pages/sessions/model-detail.tsx`). Hard contract: (1) the 4 sub-scores (`differentiation` / `gapFit` / `theoreticalSoundness` / `evidenceSupport`) always shown next to `contributionScore`; (2) per-edge `EdgeNoveltyTag` rows individually collapsible; (3) `contributionStatement === null` shows "尚未生成贡献陈述" empty state; (4) `mode === "analysis_only"` shows amber banner; `stale === true` shows stale badge. AI-emitted `contributionStatement` (7-field structured statement) shipped and populated on every `computeInnovationMeta` call. Recompute preserves `contributionStatement` + `aiReviewMarkdown`.
- **Slice 4 / #28 文献充分性检查 (shipped)**: `GET /sessions/{id}/landscape` response includes `sufficiency: SufficiencyCheck` — pure-algorithm 4-dimension rating (paperCount / recentRatio / theoryCount / coverageRate) with 木桶 principle; rule-based `suggestedSearchTerms`. Rendered as `<SufficiencyCard>` on the 文献全景 tab between the coverage banner and the CR table.
- **Phase 5 AI 评审模拟器 (pending)**: Reviewer simulator that generates structured peer-review feedback on the model's contribution claim.
...
- The 3-layer `canonicalize()` (shipped) returns `{rawName, canonicalName, contextQualifier, constructFamily}` — the existing 6-step rule is layer 2.
...
- **Shipped**: Landscape view (文献全景), Gap report, Innovation type classification, Contribution statement, Literature sufficiency check (文献充分性检查)
- **In development**: Reviewer simulator, Conversation positioning
