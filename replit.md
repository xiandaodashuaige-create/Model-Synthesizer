...
- **PNG export of live model (`pages/sessions/live-model.tsx`)**: html-to-image v1.11.13 with `skipFonts: true`, captures the `.react-flow` parent (not the viewport), temporarily mutates `viewport.transform` + flowRoot width/height to the bounding box of all nodes so off-screen content is included, awaits one rAF, restores in `finally{}`, and filters out background/controls/minimap/panel/attribution. Logs failures via `console.error`.
