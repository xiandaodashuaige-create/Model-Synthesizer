...
- **Chat ↔ Live-model bridge**: `POST /sessions/:id/model-assistant` now (a) injects a `CURRENT LIVE MODEL` block (nodes + edges) into the system prompt so the AI references real ids, and (b) parses an optional ` ```liveModelOps ` JSON block from the AI response and applies it directly to `live_model_*` tables in a transaction (`addNode`/`removeNode`/`addEdge`/`removeEdge`, validated against session-scoped variables/papers, `live_model.version` bumped on success). Response gains `liveModelApplied: { summary, applied[], rejected[], liveModelVersion }`. Frontend `model-assistant-chat.tsx` shows a sky-blue "已直接修改活跃模型" card with applied/rejected pills and invalidates `getGetLiveModelQueryKey` so the diagram refreshes immediately — no more "聊天 → 重新生成 → 再点导入" two-step. Suggestion-style regeneration is still supported in parallel via the existing `suggestion` block.
- **Global 401 interceptor**: `lib/api-client-react/src/custom-fetch.ts` dispatches a throttled (3s) `window` event `api:unauthorized` whenever any generated request returns 401. `App.tsx`'s `LoginGate` listens for it and shows a "登录已过期 / Your login expired" overlay with a 重新登录 button. This avoids the previous failure mode where an expired OIDC session silently caused every card on the page (model generation, AI usage, etc.) to render its own "失败 / Unauthorized" state with no way for the user to recover except a manual page reload.
- **Chat content as personalization material**: `refreshUserPersonalization` now fetches user-role chat *content* (capped 4000 tokens) and feeds them into `domainTokens`, so what the user actually types in chat directly shapes future generations' `topDomainKeywords` instead of only being counted as `chatTurns`.
