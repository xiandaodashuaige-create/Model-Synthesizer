...
export function generateDefenseQuestions(
  ...
  const { subScores, contributionScore, computedAgainst, mode, edgeNoveltyTags } = meta;
  ...
  // R6: novel edges with no matched relationship in the literature
  const novelEdgesWithNoMatch = edgeNoveltyTags.filter(
    (e) => e.tag === "novel" && e.matchedRelationship === null,
  );
  if (novelEdgesWithNoMatch.length > 0) {
    const names = novelEdgesWithNoMatch
      .slice(0, 3)
      .map((e) => `${e.fromVariableName} → ${e.toVariableName}`)
      .join("、");
    const suffix = novelEdgesWithNoMatch.length > 3 ? "等" : "";
    questions.push({
  ...
  // R7: analysis-only mode (coverage too low for enforcement)
  if (mode === "analysis_only") {
    ...
    // R8: coverage low but not analysis_only — warn separately
    questions.push({
      id: "coverage-low",
      question:
  ...
  // ── Sort by severity ───────────────────────────────────────────────────────
  questions.sort((a, b) => SEVERITY_ORDER[a.severity] - SEVERITY_ORDER[b.severity]);
